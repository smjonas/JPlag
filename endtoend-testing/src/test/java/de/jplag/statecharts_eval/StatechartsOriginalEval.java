...
class StatechartsOriginalEval {

    private static final List<String> TOOLS = List.of("yakindu", "scxml");

    private static final List<String> LINES_HEADER = List.of("first", "second", "language", "extraction_strategy", "min_token_length",
            "avg_similarity", "max_similarity");
    private static final List<String> LINES_HEADER_2 = List.of("year", "language", "min_token_length", "duration", "number_of_files");

    private List<String> createLine(int year, String first, String second, String language, String extractionStrategy, int minTokenLength,
            double similarity, double maxSimilarity) {
        return new ArrayList<>(List.of(Integer.toString(year), first, second, language, extractionStrategy, Integer.toString(minTokenLength),
                Double.toString(similarity), Double.toString(maxSimilarity)));
    }

    private List<String> createLine2(int year, String language, int minTokenLength, long duration, int numberOfFiles) {
        return new ArrayList<>(List.of(Integer.toString(year), language, Integer.toString(minTokenLength), Long.toString(duration),
        ...
        for (int year : new int[] {2020, 2021}) {
            for (String tool : TOOLS) {
                ...
                for (JPlagComparison tuple : jplagResult.getAllComparisons()) {
                    ...
                    lines.add(createLine(year, firstFilename, secondFilename, getToolDescriptionStrategy(tool, EXTRACTION_STRATEGY),
    ...
    public void evalBenchmark() throws ExitException {
        ...
        for (String tool : TOOLS) {
            ...
            lines.add(createLine2(year, tool.equals("scxml") ? "SCXML" : "Create", MIN_TOKEN_MATCH, avgDuration, inputFilesCount));
        }
